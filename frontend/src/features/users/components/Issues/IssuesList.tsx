        ...
        <div className="flex gap-8 border-b border-gray-200">
            {categories.map((category) => (
                <button
                    ...
                    className={`pb-4 relative transition-all duration-200 ${activeTab === category.value
                            ? "text-blue-600 font-semibold"
                            : "text-[#7C8493] text-[18px] font-semibold hover:text-gray-900"
                        }`}
                >
                    <span>
...
const TableHeader: React.FC = () => {
    return (
        <div className="grid grid-cols-12 gap-4 px-6 py-4 bg-gray-50 rounded-t-lg border-b border-gray-200">
            ...
            <div className="col-span-2 text-[18px] font-medium text-gray-600">
                Tiêu đề
            </div>
            <div className="col-span-4 text-[18px] font-medium text-gray-600">
                Nội dung
            </div>
            <div className="col-span-2 text-[18px] font-medium text-gray-600">
                Ngày đăng
    ...
    return (
        <div className="grid grid-cols-12 gap-4 px-6 py-5 border-b border-gray-100 hover:bg-gray-50 transition-colors items-center">
            ...
            <div className="col-span-2">
                <span
                    className={`inline-flex px-3 py-1 rounded-full text-sm font-medium border ${statusConfig.className}`}
                >
                    {statusConfig.text}
                </span>
            </div>
    ...
    const filteredIncidents: Incident[] = useMemo(() => {
        ...
        if (activeTab !== "all") {
            filtered = filtered.filter(
                (incident) => incident.category === activeTab
            );
        }
